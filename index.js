...
function calculateCircleArea(radius) {
  ...
  if (typeof radius !== "number") {
    console.log("Помилка: дане значення не є числом.");
    return null;
  }
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a !== "number" || typeof b !== "number") {
    console.log("Помилка: Обидві довжини катетів мають бути числами");
    return null;
  }
...
function calculateVolumeCylinder(radius, height) {
  ...
  const ceiledVolume = Math.ceil(volume);
...
function getFractionalPart(num) {
  ...
  const froundedFractionalPart = Math.fround(fractionalPart);

  return froundedFractionalPart;
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((accumulator, product) => {
    if (
      ...
      console.error(
        "Помилка: Кожен товар у масиві 'products' повинен мати допустиму властивість 'price' у вигляді числа."
      );
      return accumulator;
    }
  }, 0);
